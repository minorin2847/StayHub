...
BEGIN
    ...
    ON CONFLICT ON CONSTRAINT employees_username_key DO NOTHING;
...
DECLARE
    ...
    v_total_rows INT;
    v_offset INT;
    v_where TEXT := ' WHERE v.username <> ' || quote_literal(COALESCE(current_username, ''));
    v_query TEXT;
    v_sort_clause TEXT;
...
BEGIN
    ...
    IF p_hotel_id IS NOT NULL THEN
        v_where := v_where || format(' AND v.hotelid = %L', p_hotel_id);
    END IF;

    IF p_branch_id IS NOT NULL THEN
        v_where := v_where || format(' AND v.branchid = %L', p_branch_id);
    END IF;
    ...
    IF p_roles IS NOT NULL AND array_length(p_roles, 1) > 0 THEN
        v_where := v_where || format(
            ' AND EXISTS (
                ...
                WHERE er.employeeid = v.id
                  AND er.role = ANY(%L::text[])
            )',
            p_roles
        );
    END IF;

    EXECUTE 'SELECT COUNT(*) ' || v_base_from || v_where
    INTO v_total_rows;
...
BEGIN
    IF p_query IS NOT NULL AND p_query <> '' THEN
        v_where := v_where || format(' AND v.name ILIKE %L', '%' || p_query || '%');
    END IF;

    IF p_tier IS NOT NULL THEN
        v_where := v_where || format(' AND COALESCE(v.tier, 999) = %L', p_tier);
    END IF;
...
BEGIN
    ...
    IF p_id > 0 THEN
        v_where := v_where || format(' AND v.id = %L', p_id);
    END IF;

    IF p_name <> '' THEN
        v_where := v_where || format(' AND v.name ILIKE %L', '%' || p_name || '%');
    END IF;

    IF p_classification > 0 THEN
        v_where := v_where || format(' AND v.classification = %L', p_classification);
    END IF;

    IF p_contact_email <> '' THEN
        v_where := v_where || format(' AND v.contact_email ILIKE %L', '%' || p_contact_email || '%');
    END IF;

    IF p_contact_phone <> '' THEN
        v_where := v_where || format(' AND v.contact_phone ILIKE %L', '%' || p_contact_phone || '%');
    END IF;

    IF p_location <> '' THEN
        v_where := v_where || format(' AND v.location ILIKE %L', '%' || p_location || '%');
    END IF;
...
BEGIN
    ...
    IF p_category IS NOT NULL AND p_category <> '' THEN
        v_where := v_where || format(' AND v.category = %L', p_category);
    END IF;
...
BEGIN
    IF p_query IS NOT NULL AND p_query <> '' THEN
        v_where := v_where || format(' AND v.bed_name ILIKE %L', '%' || p_query || '%');
    END IF;
...
BEGIN
    ...
    v_where := v_where || format(' AND v.size BETWEEN %L AND %L', p_min_size, p_max_size);
    v_where := v_where || format(' AND v.capacity BETWEEN %L AND %L', p_min_capacity, p_max_capacity);
    v_where := v_where || format(' AND v.base_price BETWEEN %L AND %L', p_min_price, p_max_price);
    v_where := v_where || format(' AND v.total_beds BETWEEN %L AND %L', p_min_total_beds, p_max_total_beds);

    IF p_amenities IS NOT NULL AND array_length(p_amenities, 1) > 0 THEN
        v_where := v_where || format(
            ' AND %L::text[] <@ COALESCE(v.names_arr, ARRAY[]::text[])',
            p_amenities
        );
    END IF;
    ...
    IF p_beds IS NOT NULL AND array_length(p_beds, 1) > 0 THEN
        v_where := v_where || format(
            ' AND EXISTS (
                ...
                WHERE rtb.room_typeid = v.id
                  AND rtb.bed_name = ANY(%L::text[])
            )',
            p_beds
        );
    END IF;
...
BEGIN
    ...
    EXECUTE 'SELECT COUNT(*) FROM rooms r ' || v_where
    INTO v_total_rows;
...
BEGIN
    ...
    IF p_beds IS NOT NULL THEN
        ...
        FOR bed_record IN
            SELECT *
            FROM jsonb_to_recordset(p_beds) AS x(name TEXT, count INT)
        LOOP
            INSERT INTO beds(name)
            VALUES (bed_record.name)
            ON CONFLICT DO NOTHING;

            INSERT INTO room_type_beds (
                room_typeid,
                bed_name,
                bed_count
            )
            ...
            ON CONFLICT (room_typeid, bed_name) DO UPDATE SET
...
BEGIN
    ...
    EXECUTE 'SELECT COUNT(*) FROM vw_booking_details v ' || v_where
    INTO v_total_rows;
...
CREATE OR REPLACE FUNCTION get_reserves_by_page(
    p_query TEXT DEFAULT NULL,
    p_room_id INT DEFAULT NULL,
    p_overall_status VARCHAR DEFAULT NULL,
    p_booking_status VARCHAR DEFAULT NULL,
...
BEGIN
    IF p_query IS NOT NULL AND p_query <> '' THEN
        v_where := v_where || format(
            ' AND (v.guest_full_name ILIKE %L OR v.confirmation_codes ILIKE %L)',
            '%' || p_query || '%',
            '%' || p_query || '%'
        );
    END IF;
    ...
    EXECUTE 'SELECT COUNT(*) FROM vw_reserve_details v ' || v_where
    INTO v_total_rows;
...
BEGIN
    IF p_query IS NOT NULL AND p_query <> '' THEN
        v_where := v_where || format(' AND p.name ILIKE %L', '%' || p_query || '%');
    END IF;
    ...
    EXECUTE 'SELECT COUNT(*) FROM policies p ' || v_where
    INTO v_total_rows;
...
BEGIN
    ...
    EXECUTE 'SELECT COUNT(*) FROM services s ' || v_where
    INTO v_total_rows;
