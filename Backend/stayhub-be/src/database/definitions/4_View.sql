...
CREATE VIEW vw_reserve_details AS
SELECT r.id,
...
CREATE MATERIALIZED VIEW searchpage_view AS WITH bed_agg AS (
...
CREATE MATERIALIZED VIEW room_details_view AS WITH -- 1. Aggregate Amenities for the Room Type
...
CREATE MATERIALIZED VIEW hotel_other_rooms_view AS WITH -- 1. Aggregate Amenities (top 6 for the carousel display)
room_amenities_agg AS (
    SELECT rta.room_typeID,
        jsonb_agg(
            jsonb_build_object(
