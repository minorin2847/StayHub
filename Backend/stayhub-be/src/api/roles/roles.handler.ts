...
export async function findRole(name: string): Promise<Role> {
  const role = await db.oneOrNone("SELECT * FROM get_roles_auth_context($1)", [
    name,
  ]);
  if (!role) {
...
export function editRole(req: Request, res: Response, next: NextFunction) {
  ...
  rlsWrapper(
    ...
    async (t) => {
      ...
      const query =
        pgPromise().helpers.update(roleData, roleColumns) +
      ...
      if (newRoleName !== name) {
        await t.none("UPDATE employee_roles SET role = $1 WHERE role = $2", [
          newRoleName,
          name,
        ]);
      }
