...
passport.use(
  "user-login",
  new local.Strategy(async (username, password, cb) => {
    const row = await pg.oneOrNone(
      "SELECT id, username, hash, salt, 'user' as type FROM users WHERE username = $1",
      [username],
    );
...
passport.use(
  ...
  new local.Strategy(async (username, password, cb) => {
    const row = await pg.oneOrNone(
      "SELECT id, username, hash, salt, 'employee' as type FROM employees WHERE username = $1",
      [username],
    );
    verify(row, password, cb);
  }),
