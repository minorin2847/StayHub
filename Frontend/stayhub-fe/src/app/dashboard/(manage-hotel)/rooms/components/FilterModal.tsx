...
const EditModal = ({ open, onClose, onSuccess, current, roomTypes = [] }: any) => {
  ...
  const applyImagesToPreview = (images: RoomImage[]) => {
    ...
    setSelectedPreview(firstImage?.signed_url || firstImage?.image_url || null);
  };
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const images = await loadRoomImages(current.id);
        applyImagesToPreview(images);
      } catch (error: any) {
        console.warn(error);
        message.warning(error?.message || "Failed to load room images.");
        applyImagesToPreview([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getFirstNewFilePreview = (files = newFiles) => {
    const firstNew = files[0] as any;
  ...
  const removeNewImage = (uid: string) => {
    ...
    if (selectedPreview === targetUrl) {
      ...
      const firstNewPreview = getFirstNewFilePreview(nextNewFiles);
  ...
  const resetToOriginal = async () => {
    ...
    try {
      ...
      console.warn(error);
      message.warning(error?.message || "Failed to reset room images.");
      applyImagesToPreview([]);
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm">
                Modify room assignment and manage gallery
              </p>
            </div>
          </div>
        </div>
                      ...
                      <div className="col-span-2">
                        ...
                        >
                          <Input.TextArea
                            ...
                            placeholder="Add maintenance notes, features, or room-specific details..."
                          />
                        </Form.Item>
                      </div>
                    </div>
