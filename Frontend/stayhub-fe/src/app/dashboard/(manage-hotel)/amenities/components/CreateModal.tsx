...
export default function CreateModal({
  ...
  const fetchGlobalAmenities = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/employee/amenities/list?${params.toString()}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/employee/amenities/add-to-hotel`,
        {
          method: "POST",
  ...
  return (
    <Modal
      title={
        <div className="mb-2">
          ...
          <p className="text-sm text-slate-500 font-normal mt-1">
            Choose from existing amenities or create a new one
          </p>
        </div>
      }
      open={open}
      ...
      width={700}
      footer={[
        <Button key="reset" onClick={() => setSelectedName(null)}>
          Reset Selected
        </Button>,
