...
export default function RoleList() {
  ...
  const [currentRecord, setCurrentRecord] = useState<RoleTableData | null>(
    null,
  );
  ...
  return (
    <GenericTableView<RoleTableData, RoleFilterData>
      ...
      tableColumns={[
        ...
        },
        {
          ...
          render: (_: unknown, record: RoleTableData) => {
            ...
            return (
              <Tag
                color={tierDict[record.tier].color}
      ...
      generatedDeletePrompt={(record: RoleTableData) =>
        `Do you want to delete ${record.name}?`
      }
      generatedDeleteEndpoint={(record: RoleTableData) =>
        `${process.env.NEXT_PUBLIC_API_URL}/employee/roles/delete/${record.name}`
      }
    />
  );
}
