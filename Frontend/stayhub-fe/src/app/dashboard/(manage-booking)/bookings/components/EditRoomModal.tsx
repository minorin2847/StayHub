...
const EditRoomModal = ({ open, onClose, onSuccess, current, rooms, roomTypes }: any) => {
    const [form] = Form.useForm();
    const [roomOptions, setRoomOptions] = useState<any[]>([]);

    useEffect(() => {
        if (open && current) {
            // current is a room object from the JSONB array
            const roomInfo = rooms.find((r: any) => r.id === current.roomid);
            form.setFieldsValue({
                room_display: roomInfo?.name,
                roomID: current.roomid,
                room_status: current.status,
                room_price: current.price,
                dates: [dayjs(current.checkin), dayjs(current.checkout)]
            });
        }
    }, [open, current, rooms]);
    ...
    const handleFinish = async (values: any) => {
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/employee/bookings/rooms/edit/${current.id}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    roomID: values.roomID,
                    room_status: values.room_status,
                    room_price: values.price,
    ...
    return (
        <Modal 
            title={<span><HomeOutlined /> Edit Room Stay</span>} 
            open={open} 
            onCancel={onClose} 
            onOk={form.submit} 
            width={600}
        ...
        >
            <Form form={form} layout="vertical" onFinish={handleFinish} className="mt-4">
                ...
                <Row gutter={16}>
                    ...
                    <Col span={12}>
                        <Form.Item name="room_status" label="Room Status" rules={[{required: true}]}>
                            <Select>
